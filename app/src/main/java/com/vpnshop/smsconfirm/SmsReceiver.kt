...
/**
 * فقط پیامک‌هایی که فرستنده‌شان شامل یکی از الگوهای فیلتر تنظیم‌شده باشد
 * (پیش‌فرض: "Pasargad" - می‌شود چند الگو را با ویرگول جدا کرد، مثلاً برای
 * وقتی چند بانک روی یک سیم‌کارت پیامک می‌فرستند: "Pasargad,Bank Mellat")
...
class SmsReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (filter.isNotBlank()) {
            // پشتیبانی از چند بانک روی یک سیم‌کارت: می‌شود چند الگو را با ویرگول جدا نوشت
            // (مثلاً "Pasargad,Bank Mellat") - اگر فرستنده با هیچ‌کدام match نکرد، رد می‌شود.
            val patterns = filter.split(",").map { it.trim() }.filter { it.isNotEmpty() }
            if (patterns.isNotEmpty() && patterns.none { sender.contains(it, ignoreCase = true) }) {
                // فرستنده با هیچ‌کدام از بانک‌های تنظیم‌شده match نکرد - رد می‌شود، جایی ذخیره نمی‌شود
                return
            }
        }
